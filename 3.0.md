...
The asynchronous request methods now use the Node.js callback pattern, where the first argument to the callback is the error (rather than the response content). This makes d3.xhr compatible with asynchronous JavaScript helper libraries, such as [Queue.js](/mbostock/queue).
